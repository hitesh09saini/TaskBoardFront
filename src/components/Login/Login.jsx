...
const Login = ({ loader }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    loader(true);
    try {
      const res = await instance.post('/api/v1/user/login', { email, password });
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="mb-5">
            ...
            <input
              ...
              autoComplete="email"
              required
              className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 ${loginError ? 'border-red-500' : 'border-gray-300'
                }`}
          ...
          {loginError && <p className="mb-4 text-red-600 text-sm">{loginError}</p>}
